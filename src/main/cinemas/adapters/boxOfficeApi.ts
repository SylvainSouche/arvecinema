// ──────────────────────────────────────────────────────────────────────────
// boxofficeapi adapter plugin
// ──────────────────────────────────────────────────────────────────────────
// Shared by every cinema whose website is built on the same Gatsby +
// gatsby-source-boxofficeapi stack (Mont-Blanc, Cluses, Bonneville). All
// that differs between them is the base URL + theater ID — those are passed
// in via the cinema's config object so a new cinema is a one-line change in
// `cinemas.config.json`.
//
...
//     "theaterId": "P1798",
//     "timeZone": "Europe/Paris"    // optional, defaults to Europe/Paris
//   }
// ──────────────────────────────────────────────────────────────────────────
...
const plugin: CinemaAdapterPlugin = {
  id: 'boxofficeapi',
  displayName: 'Box Office API',
  description:
    'Gatsby + gatsby-source-boxofficeapi stack (cinemontblanc.fr, cine-cluses.fr, cinechateau.fr)',
  ...
  validateConfig(config: CinemaConfig): ConfigValidationResult {
    ...
    try {
      requireString(config, 'theaterId');
    } catch (e) {
      errors.push(e instanceof Error ? e.message : String(e));
    }
  ...
  createAdapter(cinemaId: string, config: CinemaConfig): CinemaAdapter {
    ...
    const theaterId = requireString(config, 'theaterId');
    const timeZone = optionalString(config, 'timeZone', TIMEZONE);
    ...
    return {
      async fetchSchedule(windowDays: number) {
        ...
        const params = new URLSearchParams({
          ...
          to: to.toISOString(),
          theaters: JSON.stringify({ id: theaterId, timeZone }),
        });

        const schedRes = await withNetworkTracking(() =>
          fetchWithTimeout(
            `${baseUrl}/api/gatsby-source-boxofficeapi/schedule?${params}`,
            { headers },
            REQUEST_TIMEOUT_MS,
          ),
        );
        ...
        const theaterData = (raw as Record<string, any>)[theaterId];
        ...
        if (skippedShowtimes > 0) {
          log.cinemas.warn(
            `[cinemas] ${cinemaId}: skipped ${skippedShowtimes} malformed showtime(s)`,
          );
        }
        ...
        if (moviesById.size > 0) {
          ...
          for (let i = 0; i < allIds.length; i += BATCH_SIZE) {
            ...
            try {
              const metaRes = await withNetworkTracking(() =>
                fetchWithTimeout(
                  `${baseUrl}/api/gatsby-source-boxofficeapi/movies?${metaParams}`,
                  { headers },
                  REQUEST_TIMEOUT_MS,
                ),
              );
              if (metaRes.ok) {
                ...
                if (Array.isArray(metaList)) {
                  for (const m of metaList as any[]) {
                    ...
                    local.runtime =
                      typeof m.runtime === 'number' ? Math.round(m.runtime / 60) : undefined;
                    // Store external IDs (altId) so the ratings enricher can
                    // look up AlloCiné IDs for MPDB.
                    if (Array.isArray(m.altId)) {
                      (local as Movie & { altId?: string[] }).altId = m.altId.filter(
...
// ── Backward-compatible factory (used by the old registry during migration) ──
...
export function createBoxOfficeApiAdapter(
  ...
  return plugin.createAdapter(cinemaId, { kind: 'boxofficeapi', ...cfg });
}
