...
export function createCineChateauAdapter(cinemaId: string, cfg: CineChateauConfig): CinemaAdapter {
  ...
  return {
    async fetchSchedule(windowDays: number) {
      ...
      const html = await browserFetch(`${cfg.baseUrl}${schedulePath}`, { headers: HEADERS });
      ...
      if (!html.includes('hr_film')) {
        log.cineChateau.error(
          `page does not contain .hr_film blocks (got ${html.length} bytes). First 500 chars: ${html.substring(0, 500)}`,
        );
        throw new ScraperSchemaChangedError(
      ...
      const availableDays = allParsedDays.filter((d) => d >= today && d <= windowEnd).sort();
