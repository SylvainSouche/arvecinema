...
import { t, getLocale, formatDecimal } from '../../shared/i18n';

/** Pick a color for a 0-100 score. */
const scoreColor = (score: number | undefined): string => {
  if (score === undefined) return 'var(--text-faint)';
  if (score >= 75) return 'var(--brand-success)'; // green — fresh
  if (score >= 60) return 'var(--brand-warning)'; // yellow — meh
  return 'var(--brand-error)'; // red — rotten
};
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div
          style={{ display: 'flex', gap: 6, marginTop: 2, alignItems: 'center', flexWrap: 'wrap' }}
        >
        ...
        <div
          style={{ display: 'flex', gap: 8, marginTop: 2, alignItems: 'center', flexWrap: 'wrap' }}
        >
          {movie.hasVF && (
            <span
              style={{
          ...
          {(movie.imdbId ||
            ...
            movie.wikidataUrl === undefined) && <RatingBadges movie={movie} />}
        </div>
 ...
 *  - ok      → green (rating fetched)
 *  - absent  → grey  (source has no rating for this film — legitimate, not a failure)
 *  - blocked → red   (fetch failed: Cloudflare, network, parse error — potentially retriable) */
const statusColor = (status: string | undefined): string => {
  if (status === 'ok') return 'var(--brand-success)';
  if (status === 'blocked') return 'var(--brand-error)';
  if (status === 'absent') return 'var(--text-muted)';
  return 'var(--text-faint)'; // unknown / loading
};

/** One-word label for a status, used in the title tooltip. */
const statusLabel = (status: string | undefined): string => {
  if (status === 'ok') return t('imdbRating');
  if (status === 'blocked') return t('ratingBlocked');
  if (status === 'absent') return t('ratingAbsent');
  return t('ratingPending');
};
  ...
  const StatusBadge: React.FC<{
    ...
    const status =
      source === 'imdb'
        ? movie.imdbStatus
        : source === 'allocine'
          ? movie.allocineStatus
          : movie.rtStatus;
    const msg =
      source === 'imdb'
        ? movie.imdbStatusMessage
        : source === 'allocine'
          ? movie.allocineStatusMessage
    ...
    // Status is 'absent' (legitimate — source has no rating) or 'blocked'
    // (error — fetch failed). Distinguish visually:
    //   absent  → grey dot + "—" (no retry needed)
    //   blocked → red dot + "⚠" (retry might help)
    const isError = status === 'blocked';
    const symbol = isError ? '⚠' : '–';

    return (
      <span
        title={`${label} — ${statusLabel(status)}${msg ? `: ${msg}` : ''}`}
        style={{
          display: 'inline-flex',
          alignItems: 'center',
          gap: 2,
        ...
        <>
          ...
          {movie.imdbStatus === 'ok' && movie.imdbRating !== undefined ? (
            <button
              type="button"
              onClick={() => {
                if (movie.imdbUrl) void window.electronAPI.openTicket(movie.imdbUrl);
              }}
              title={[
                t('imdbRating'),
                `${t('ratingLabel')} : ${movie.imdbRating.toFixed(1)}/10`,
                movie.imdbVotes !== undefined
                  ? `${movie.imdbVotes.toLocaleString(getLocale() === 'fr' ? 'fr-FR' : 'en-US')} ${t('votes')}`
                  : null,
                movie.imdbUrl ? '\n' + t('openImdb') : null,
              ]
                .filter(Boolean)
          ...
          {movie.allocineStatus === 'ok' &&
          (movie.allocinePress !== undefined || movie.allocineAudience !== undefined) ? (
            <button
              type="button"
              onClick={() => {
                if (movie.allocineUrl) void window.electronAPI.openTicket(movie.allocineUrl);
              }}
              title={[
                'AlloCiné',
                movie.allocinePress !== undefined
                  ? `${t('pressRating')} : ${formatDecimal(movie.allocinePress)}/5`
                  : null,
                movie.allocineAudience !== undefined
                  ? `${t('audienceRating')} : ${formatDecimal(movie.allocineAudience)}/5`
                  : null,
          ...
          {movie.rtStatus === 'ok' && movie.rtTomatometer !== undefined ? (
            <button
              type="button"
              onClick={() => {
                if (movie.rtUrl) void window.electronAPI.openTicket(movie.rtUrl);
              }}
              title={[
